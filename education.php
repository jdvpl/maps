<?php 
	...
	class education	{
		...
		function setDepartamento($departamento) { $this->departamento = $departamento; }
		...
		function setDaneSede($dane_sede) { $this->dane_sede = $dane_sede; }
		function getDaneSede() { return $this->dane_sede; }

		function setInstitucionEducativa($institucion_educativa) { $this->institucion_educativa = $institucion_educativa; }
		function getInstitucionEducativa() { return $this->institucion_educativa; }

		function setDaneInstitucion($dane_institucion) { $this->dane_institucion = $dane_institucion; }
		function getDaneInstitucion() { return $this->dane_institucion; }

		function setBaseOrigen($base_origen) { $this->base_origen = $base_origen; }
		function getBaseOrigen() { return $this->base_origen; }
		...
		function setFormacion($formacion) { $this->formacion = $formacion; }
		function getFormacion() { return $this->formacion; }
		...
		public function getDepartamentos() {
			$sql = "SELECT DISTINCT departamento FROM  $this->tableName";
			$stmt = $this->conn->prepare($sql);
			$stmt->execute();
			return $stmt->fetchAll(PDO::FETCH_ASSOC);
		}
		...
		public function getMunicipios() {
			...
			$sql = "SELECT DISTINCT municipio FROM  $this->tableName WHERE departamento = 'HUILA'";
			$stmt = $this->conn->prepare($sql);
			$stmt->execute();
			return $stmt->fetchAll(PDO::FETCH_ASSOC);
		}
